...
And thus the transition matrix $\mathbf{X}_{k-1}$ is given by 
\begin{equation}
...
Suppose a linear system: $\dot{x}(t) = Ax(t) + b$ where $A$ is a constant matrix and $b$ is a constant vector. We found the exact solution of $x(t+T)$ as:
\begin{equation}
...
\begin{equation}
    \mathbf{x}(t+T) = exp(\mathbf{\tilde{A}}T)\mathbf{x}(t) + \left( IT + \frac{\mathbf{A}T^2}{2} + \frac{\mathbf{A}^2T^3}{3!} + \dots \right) \tau
\end{equation}
