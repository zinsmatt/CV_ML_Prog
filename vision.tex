...
\end{equation}
Because $f$ is non-linear, we need to use a first-order approximation, using its Jacobian.

\begin{equation}
    C_{f(X)} = J_f(X) C_X J_f(X)^T
\end{equation}

This jacobian $J_f(X)$ can be obtained with chain-rule.

\begin{equation}
    J_f(X) = J_{f_{intr}}(x_n) J_{f_{proj}}(X_{cam}) J_{f_{Rt}}(X)
\end{equation}

where each sub-Jacobian is

\begin{equation}
   J_{f_{intr}}(x_n) = \left[\begin{array}{cc}
       f_x & 0 \\
        0 & f_y
   \end{array}\right]
\end{equation}
...
\begin{equation}
   J_{f_{proj}}(X_{cam}) = \left[\begin{array}{ccc}
