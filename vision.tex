...
In that case, we have no supposition on the covariance of the noise. The noise has $mean=0$ and its covariance is $\Sigma_{\epsilon}$. The estimator is still
\begin{equation}
    \hat{\beta} = (X^T X)^{-1}X^Ty
\end{equation}
...
\begin{equation}
    \Sigma_{\hat{\beta}} = (X^T X)^{-1} X^T \Sigma_{\epsilon} X (X^T X)^{-1}
\end{equation}
In fact, this seems to be the same as the method from "the Bible": $(J^T \Sigma^{-1} J)^{-1}$.
